...
from auth import router as auth_router, get_user_from_token, run_demo_cleanup, run_pending_signup_cleanup
...
def cron_check_reminders():
    # Demo cleanup + expired pending-signup cleanup both piggyback on this
    # same tick — it's already pinged every minute by cron-job.org, so no
    # separate cron job is needed for either. Both queries are cheap (one
    # SELECT/DELETE each, do nothing if nothing's due).
    run_demo_cleanup()
    run_pending_signup_cleanup()
